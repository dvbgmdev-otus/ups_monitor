...
Основной способ работы с проектом — команды `./prj`. При этом сборка, запуск, тестирование, статический анализ и формирование пакета выполняются в Docker-контейнере с окружением целевой платформы.

При необходимости проект и эмулятор можно собрать и запустить непосредственно на локальной системе с помощью CMake. Этот сценарий описан в разделе «Локальная проверка с эмулятором».

## Запуск в контейнере

Проверить интерфейс командной строки без подключения к ИБП можно командой:

```bash
./prj run --help
```

Для мониторинга нужен реальный ИБП или SNMP-эмулятор, доступный из контейнера. Адрес по умолчанию `127.0.0.1` относится к самому контейнеру, поэтому запущенный на хосте эмулятор по этому адресу недоступен. Воспроизводимый запуск с локальным эмулятором описан в разделе «Локальная проверка с эмулятором».
